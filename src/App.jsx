...
async function getDetails(candidate, directorHint) {
  ...
  try {
    ...
    const query = tmdbImdbId ? `${PROXY}/omdb?imdbId=${tmdbImdbId}` : `${PROXY}/omdb?title=${encodeURIComponent(title)}&year=${year}`;
    const omdb = await (await fetch(query)).json();
...
function Fade({ children }) {
  ...
  return (
    <div style={{ opacity: vis ? 1 : 0, transition: "opacity 0.2s ease" }}>
      {children}
    </div>
  );
}
...
function AddModal({ onClose, onAdd, onUpdate, existingFilms, editFilm }) {
  ...
  const [form, setForm] = useState(isEdit ? {
    ...
    year: editFilm.year || "",
    genre: editFilm.genre || "",
    director: editFilm.director || "",
    country: editFilm.country || "",
    actors: editFilm.actors || "",
    awards: String(editFilm.awards ?? ""),
    imdbRating: editFilm.imdbRating || "",
    imdbId: editFilm.imdbId || "",
    poster: editFilm.poster || "",
    backdrop: editFilm.backdrop || "",
    plot: editFilm.plot || "",
    runtime: editFilm.runtime || ""
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      if (!results.length) {
        setErr("No results found. Try different terms or fill in manually.");
        ...
        <div style={{ background:"#0f0f1e", border:"1px solid #2a2a4a", borderRadius:14, padding:"1.5rem", width:"min(560px, 94vw)", maxHeight:"90vh", overflowY:"auto", color:"#fff" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.25rem" }}>
            <div style={{ fontSize:17, fontWeight:500 }}>
              {isEdit ? `Edit — ${editFilm.title}` : step==="search" ? "Add a film" : step==="confirm" ? "Select the correct film" : "Review details"}
            </div>
          ...
          {step==="edit" && (
            <>
              <div style={{ fontSize:12, color:"#8888aa", marginBottom:"0.75rem", lineHeight:1.5 }}>
                {isEdit ? "Update any field and save." : "Review all fields before saving."}
              </div>
              {err && <div style={{ fontSize:12, color:"#ff8888", marginBottom:8 }}>{err}</div>}
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  ...
                  <button onClick={handleSave} style={{ flex:2, background:"#7F77DD", color:"#fff", border:"none", borderRadius:8, padding:"10px", fontWeight:500, cursor:"pointer", fontSize:14 }}>
...
function AppInner() {
  ...
  const topForeignCountry = (() => {
    const counts = {};
    watchedFilms.forEach(f => { if (f.country && f.country !== "United States" && f.country !== "USA") counts[f.country] = (counts[f.country]||0)+1; });
  ...
  const mostAwardedDirector = (() => {
    const counts = {};
    watchedFilms.forEach(f => { if (f.director && f.awards > 0) counts[f.director] = (counts[f.director]||0) + Number(f.awards); });
